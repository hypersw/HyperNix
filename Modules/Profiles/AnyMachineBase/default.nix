    ...
    (lib.mkIf cfg.enable {
      ...
      system.activationScripts.localFlake = lib.mkIf cfg.localFlake.enable (
        ...
        in ''
          ...
          {
            description = '''
          Multi-input shim with parallel main and candidate trios. Each trio is `{nixpkgs, nixos-hardware, upstream}`. The main trio drives `nixosConfigurations.default` (production); the candidate trio drives `nixosConfigurations.candidate` (the try-boot staging slot — see Modules/System/BootOnce/). Every input uses a moving branch-ref URL (nixos-unstable for the nixpkgs / nixos-hardware pair, the upstream repo's HEAD for the HyperNix snapshot); each input's `original` block descriptor is identical across the two trios, but each input locks to a separate rev. Result: main and candidate hold independent revisions of the same branches at the same time.
          ...
          sudo nix flake update nixpkgs-candidate nixos-hardware-candidate upstream-candidate /etc/nixos
              Starts a fresh major-upgrade probe at current nixos-unstable HEAD without touching production.
          ...
          Structurally copies nodes.nixpkgs-candidate.locked over nodes.nixpkgs.locked (same for nixos-hardware) inside /etc/nixos/flake.lock and offers a 1-4 apply prompt (nothing / switch / boot / boot + reboot). See Modules/System/BootOnce/.
            ''';
