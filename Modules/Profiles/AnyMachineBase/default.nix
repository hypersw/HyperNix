    ...
    (lib.mkIf cfg.enable {
      ...
      system.activationScripts.localFlake = lib.mkIf cfg.localFlake.enable (
        ...
        in ''
          ...
          #                    upstream-candidate
          #     Refresh the candidate trio to current nixos-unstable HEAD.
          #     Manual; used to start a major-upgrade probe.
          #
          ...
          # preserved so subsequent `nix flake update nixpkgs` advances
          # to current nixos-unstable HEAD normally:
          #
          #   Edit /etc/nixos/flake.lock by hand:
          #     nodes.nixpkgs-candidate.locked
          #       -> nodes.nixpkgs.locked
          #     nodes.nixos-hardware-candidate.locked
          #       -> nodes.nixos-hardware.locked
          #   Leave each main node's `original` block untouched.
          FLAKE
