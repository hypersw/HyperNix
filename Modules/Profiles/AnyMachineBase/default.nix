    ...
    (lib.mkIf cfg.enable {
      ...
      system.activationScripts.localFlake = lib.mkIf cfg.localFlake.enable (
        ...
        in ''
          ...
          # This file is regenerated by the activation script on every
          # nixos-rebuild switch — content is module-owned. Operator
          # edits go through Modules/Profiles/AnyMachineBase/default.nix's
          # local-flake template, NOT through this file. /etc/nixos/flake.lock
          # is operator state (mutated by nix flake update, promote-candidate,
          # etc.) and is left alone.
          #
          # Multi-input shim with parallel main and candidate trios. Each
          # trio is `{nixpkgs, nixos-hardware, upstream}`. The main trio
          # drives `nixosConfigurations.default` (production); the
          # candidate trio drives `nixosConfigurations.candidate` (the
          # try-boot staging slot — see Modules/System/BootOnce/). Every
          # input uses a moving branch-ref URL (nixos-unstable for the
          # nixpkgs / nixos-hardware pair, the upstream repo's HEAD for
          # the HyperNix snapshot); each input's `original` block
          # descriptor is identical across the two trios, but each input
          # locks to a separate rev. Result: main and candidate hold
          # independent revisions of the same branches at the same time.
          #
          # Lock-update commands (each touches only the named inputs;
          # others are left exactly as they were):
          #
          #   nix flake update upstream
          #     Bump the production HyperNix snapshot. Wired into the
          #     ~5-minute auto-rebuild-on-push timer.
          #
          #   nix flake update
          #     Advance every input (both trios + upstream) to current
          #     branch HEAD. Wired as the preStart of nixos-upgrade.service
          #     (monthly or per-cadence timer). The candidate trio
          #     auto-rolls forward along with the production trio at the
          #     monthly tick — promote or discard within the cadence
          #     window if you want a probe to outlive that.
          #
          #   nix flake update nixpkgs-candidate nixos-hardware-candidate \
          #                    upstream-candidate
          #     Start a fresh major-upgrade probe at current
          #     nixos-unstable HEAD without touching production. Manual.
          #
          # Try-boot the candidate (Pi 5 EEPROM one-shot; failure auto-
          # reverts on the next power cycle):
          #
          #   sudo nixos-rebuild-boot-once --flake /etc/nixos#candidate
          #   sudo reboot-tryboot
          #
          # Promote a successful candidate without any rebuild — full
          # local cache reuse, branch-ref descriptors on the main trio
          # preserved so subsequent monthly upgrades advance normally:
          #
          #   sudo nixos-rebuild-promote-candidate
          #
          # Structurally copies nodes.nixpkgs-candidate.locked over
          # nodes.nixpkgs.locked (same for nixos-hardware) inside
          # /etc/nixos/flake.lock and offers a 1-4 apply prompt
          # (nothing / switch / boot / boot + reboot). See
